...
enum dfpu17_mode { MODE_OFF, MODE_INT, MODE_POLL };

enum dfpu17_prec { PREC_SINGLE, PREC_DOUBLE, PREC_HALF };

enum dfpu17_status { STATUS_OFF, STATUS_IDLE, STATUS_RUNNING, STATUS_WAITING };
...
void dfpu17_swap_buffers(struct device *device) {
	...
	memcpy(temporary, dfpu17_get(device, mem.word.data0), 512 * sizeof(u16));

	memcpy(dfpu17_get(device, mem.word.data0), dfpu17_get(device, mem.word.data1),
		   512 * sizeof(u16));

	memcpy(dfpu17_get(device, mem.word.data1), temporary, 512 * sizeof(u16));
}
...
#define CMP(O, A, B)                                                                               \
	(!((PREC == PREC_HALF)	   ? (HREG[A] O HREG[B])                                               \
...
static void print_state(struct hardware *hw, u16 instruction) {
	...
	fprintf(stderr, "status=%d, loadstatus=%d, error=%d\n", dfpu17_get(hw->device, status),
			dfpu17_get(hw->device, loadstatus), dfpu17_get(hw->device, error));
...
struct device *make_dfpu17(struct dcpu *dcpu) {
	...
	struct device d = {
		0x1de171f3, 0x0001, 0x5307537, NULL, &dfpu17_interrupt, &dfpu17_cycle,
	};
