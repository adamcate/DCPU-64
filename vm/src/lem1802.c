...
enum lem1802_command { MEM_MAP_SCREEN, MEM_MAP_FONT, MEM_MAP_PALETTE, SET_BORDER_COLOUR };
...
const u16 lem1802_zardoz_font[256] = {
	...
	0x7149, 0x4700, 0x007f, 0x4100, 0x031c, 0x6000, 0x417f, 0x0000, 0x0201, 0x0200, 0x8080, 0x8000,
...
const u16 lem1802_default_12bit_palette[] = {0x0000, 0x000a, 0x00a0, 0x00aa, 0x0a00, 0x0a0a,
											 0x0a50, 0x0aaa, 0x0555, 0x055f, 0x05f5, 0x05ff,
											 0x0f55, 0x0f5f, 0x0ff5, 0x0fff};

const u16 lem1802_default_16bit_palette[] = {0x0000, 0x0015, 0x0540, 0x0555, 0xa800, 0xa815,
											 0xad40, 0xad55, 0x52aa, 0x52bf, 0x57ea, 0x57ff,
...
#define LEM1802_FF_PIXSIZE                                                                         \
	(sizeof(struct farbfeld_pixel) * LEM1802_FF_PIXWIDTH * LEM1802_FF_PIXHEIGHT)
#define LEM1802_FF_SIZE (sizeof(struct farbfeld_data) + LEM1802_FF_PIXSIZE)
#define LEM1802_SCALE_FACTOR 4

static const struct farbfeld_data lem1802_ff_init = {LEM1802_FF_PIXWIDTH, LEM1802_FF_PIXHEIGHT};
...
#define COLOUR(x)                                                                                  \
	(get_member_of(struct device_lem1802, hw->device, use_16bit_colour)                            \
		 ? colour_16bit(palette, (x))                                                              \
		 : colour_12bit(palette, (x)))

void lem1802_render(struct farbfeld_pixel *pixels, SDL_Window *window);

void lem1802_draw_char(struct hardware *hw, struct farbfeld_pixel *pixels, int i, int j, u16 vram,
					   const u16 font[256], const u16 palette[16], int cycle) {
	int x, y;
	u8 bg = (vram >> 8) & 0xf;
	u8 fg = (vram >> 12) & 0xf;
	u8 blink = (vram >> 7) & 0x1;
...
void lem1802_draw(struct hardware *hw, struct farbfeld_pixel *pixels, const u16 vram[386],
				  const u16 font[256], const u16 palette[16], u8 bordercol, int cycle) {
	int i, j, k;
	...
	for (i = 0; i < LEM1802_FF_ROWS; i++) {
		for (j = 0; j < LEM1802_FF_COLS; j++) {
			lem1802_draw_char(hw, pixels, i, j, vram[i * LEM1802_FF_COLS + j], font, palette,
							  cycle);
		}
	}
}
...
void lem1802_render(struct farbfeld_pixel *pixels, SDL_Window *window) {
	...
	surface = SDL_CreateRGBSurfaceFrom((void *)pixels, LEM1802_FF_PIXWIDTH, LEM1802_FF_PIXHEIGHT,
									   3 * 8, 3 * LEM1802_FF_PIXWIDTH, 0x0000ff, 0x00ff00, 0xff0000,
									   0);
...
#define WINDOW get_member_of(struct device_lem1802, hw->device, window)
	...
	if (paletteoff == 0)
		if (get_member_of(struct device_lem1802, hw->device, use_16bit_colour))
			throw("use_16bit_colour",
				  "unimplemented"); /* palette = lem1802_default_16bit_palette; */
		else
	...
	if (WINDOW == NULL) {
		...
		**ffdat = LEM1802_FF_INIT;
		...
		WINDOW = SDL_CreateWindow("LEM1802", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
								  LEM1802_SCALE_FACTOR * LEM1802_FF_PIXWIDTH,
								  LEM1802_SCALE_FACTOR * LEM1802_FF_PIXHEIGHT, 0);
		if (WINDOW == NULL) {
	...
	if (is_dirty) {
		...
		lem1802_draw_char(hw, (*ffdat)->pixels, i, j, vram[i * LEM1802_FF_COLS + j], font, palette,
						  is_cycle);
	}
...
void lem1802_interrupt(struct hardware *hw, struct dcpu *dcpu) {
	switch (dcpu->registers[0]) {
		case MEM_MAP_SCREEN:
			get_member_of(struct device_lem1802, hw->device, vramoff) = dcpu->registers[1];
			break;
		case MEM_MAP_FONT:
			get_member_of(struct device_lem1802, hw->device, fontoff) = dcpu->registers[1];
			break;
		case MEM_MAP_PALETTE:
			get_member_of(struct device_lem1802, hw->device, paletteoff) = dcpu->registers[1];
			break;
		case SET_BORDER_COLOUR:
			get_member_of(struct device_lem1802, hw->device, bordercol) = dcpu->registers[1];
			break;
	}
}
...
struct device *make_lem1802(struct dcpu *dcpu) {
	...
	{
		struct device d = {
			0x7349f615, 0x1802, 0x1c6c8b36, NULL, &lem1802_interrupt, &lem1802_cycle,
		};
		d.data = lem1802;
